...
impl Map {
    ...
    pub fn search_min_cost(&self, print_progress: bool) -> Option<Cost> {
        if self.is_solved() {
            return Some(0);
        }
        ...
        let mut moved = false;
        let mut i = 0;
        let nexts = self.get_next_maps();
        for (next_map, next_cost) in &nexts {
            if print_progress {
                i += 1;
                println!("searching... {} / {}", i, nexts.len());
            }
            if let Some(after_cost) = next_map.search_min_cost(false) {
...
fn main() {
    ...
    println!("{:?}", map.search_min_cost(true));
}
